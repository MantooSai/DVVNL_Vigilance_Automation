...
import com.demo.database.AssessmentApprovalCaseDataBase;
import com.demo.pom.AssessmentApprovalPage;
...
public class AssessmentApproval  extends BaseClass{
	...
	@Test(priority = 2)
	public void toverifyHeaderNameAssessmentApprovalCase() {
		hp=new HomePage(driver);
	...
	public void toveriyCountOfAssessmentApprovalCase() {
		...
		try {
			...
			logger.info("Get Total Count from UI:- "+totalCountOnUI);
			String totalCountOnDB = apcd.toverifyCountAssessmentApprovalCaseInDatabase();
			logger.info("Get Total Count from UD:- "+totalCountOnDB);
	...
	@Test(priority = 4)
	public void toveriyAssessmentApprovalCaseDetailsFromDatabase() {
		hp=new HomePage(driver);
		cdp=new CommonDataPage(driver);
		aap=new AssessmentApprovalPage(driver);
		rc=new RobotClass();
		...
		try {
			...
			List<String> assessmentApprovalCaseDetailsOnUI = aap.allDetails();
			  logger.info("Get all Record from UI :- "+assessmentApprovalCaseDetailsOnUI);
			List<String> assessmentApprovalCaseDetailsOnDB = apcd.toverifyAssessmentApprovalCaseDetailsInDatabase();
			  logger.info("Get all Record from UD :- "+assessmentApprovalCaseDetailsOnDB);
			  Assert.assertEquals(assessmentApprovalCaseDetailsOnUI, assessmentApprovalCaseDetailsOnDB);
